...
const messages = {
    ...
    TWEET_RESOLVE_ID: (action) => `Could not resolve the tweet ID to ${action}.`,
    INVALID_FETCH_OPTIONS: 'Could not fetch the requested data due to supplied options being invalid or incomplete.',
    USER_RESOLVE_ID: (action) => `Could not resolve the user ID to ${action}.`,
    ...
    SPACE_RESOLVE_ID: (action) => `Could not resolve the space ID to ${action}.`,
    LIST_RESOLVE_ID: (action) => `Could not resolve the list ID to ${action}.`,
    RULE_NOT_FOUND: `The specified rule does not exist or has been deleted`,
    RULE_RESOLVE_ID: (action) => `Could not resolve the rule ID to ${action}.`,
};
for (const [key, message] of Object.entries(messages)) {
    (0, TwitterjsError_1.regsiterErrorMessage)(key, message);
}
